...
// Konfigurasi path audio (UBAH SESUAI FOLDER ANDA)
audioLoader.setAudioPaths({
  menu: 'audio/menu-bgm.mp3',      // ← Taruh file MP3 di folder "audio"
  gameplay: 'audio/gameplay-bgm.mp3',
  boss: 'audio/boss-bgm.mp3',
  victory: 'audio/victory-bgm.mp3',
  gameover: 'audio/gameover-bgm.mp3'
});

// ========== MODIFY PLAY GAME FUNCTION ==========
function playGame() {
  audioLoader.fadeOut(500); // Fade out menu music
  window.location.href = 'game.html';
}

// ========== MODIFY SETTINGS FUNCTIONS ==========
function toggleBGM() {
  gameSettings.toggleBGM();
  
  if (gameSettings.bgm) {
    audioLoader.play('menu', true);
  } else {
    audioLoader.stop();
  }
  ...
  requestAnimationFrame(frame);
  // ========== INITIALIZATION - UPDATE DOMContentLoaded ==========
window.addEventListener('DOMContentLoaded', () => {
  generateStars();
  updateStatsDisplay();
  
  // ✅ Play menu music jika BGM enabled
  if (gameSettings.bgm) {
    audioLoader.play('menu', true);
    audioLoader.fadeIn(1000);
  }
});
